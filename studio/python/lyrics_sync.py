"""Local known-lyrics alignment for HeartMuLa library songs."""
...
if not RUNTIME_PYTHON.is_file() and os.name == "nt":
    RUNTIME_PYTHON = Path(r"F:\MiniMaxM3\python\lyrics_runtime\Scripts\python.exe")
WORKER = ROOT / "python" / "lyrics_align_worker.py"
MODEL_ROOT = ROOT / "models" / "lyrics"
if not MODEL_ROOT.is_dir():
    MODEL_ROOT = Path(r"F:\MiniMaxM3\models\lyrics")
...
def status() -> dict[str, Any]:
    ...
    return {
        ...
        "detail": (
            "GPU lyric timing is ready. Language models download into the HeartMuLa models folder on first use."
