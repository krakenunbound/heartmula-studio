...
log = logging.getLogger("music3.sfx")


MODEL_ROOT = ROOT / "models" / "sound_effects" / "stable-audio-3-small-sfx"
if not (MODEL_ROOT / "model.safetensors").is_file():
    MODEL_ROOT = Path(r"F:\MiniMaxM3\models\sound_effects\stable-audio-3-small-sfx")
...
if not RUNTIME_PYTHON.is_file():
    RUNTIME_PYTHON = Path(r"F:\MiniMaxM3\python\sfx_runtime\Scripts\python.exe")
    RUNTIME_PACKAGE = Path(r"F:\MiniMaxM3\python\sfx_runtime\Lib\site-packages\stable_audio_3")
...
def status() -> dict[str, Any]:
    ...
    if missing:
        ...
        detail = "Stable Audio 3 Small SFX ready · local CPU generation · HeartMuLa stays loaded"
    return {
