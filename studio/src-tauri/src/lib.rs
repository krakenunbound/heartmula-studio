//! HeartMuLa Studio host: owns and supervises the local Python sidecar.
...
fn project_root() -> PathBuf {
    ...
    let mut candidates = vec![cwd.clone(), cwd.join("studio")];
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        if let Some(parent) = exe.parent() { candidates.push(parent.join("studio")); }
    }
    candidates
        .into_iter()
...
fn venv_python() -> PathBuf {
    let local = project_root().join("python").join("venv").join("Scripts").join("python.exe");
    if local.is_file() { return local; }
    // Reuse the already-present companion app runtime; no new installation is required.
    for fallback in [
        PathBuf::from(r"F:\MiniMaxM3\python\venv\Scripts\python.exe"),
        PathBuf::from(r"F:\Kraken ACE\python\venv\Scripts\python.exe"),
    ] {
...
fn sidecar_healthy() -> bool {
    ...
    payload.get("ok").and_then(serde_json::Value::as_bool) == Some(true)
        && payload.get("service").and_then(serde_json::Value::as_str) == Some("HeartMuLa Studio")
...
fn spawn_sidecar() -> Result<Child, String> {
    ...
    let mut child = hidden_command(&python)
        ...
        .env("PYTHONPATH", r"F:\MiniMaxM3\python\runtime\Lib\site-packages")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|error| format!("sidecar spawn failed: {error}"))?;
    if let Some(out) = child.stdout.take() {
        thread::spawn(move || for line in BufReader::new(out).lines().map_while(Result::ok) { eprintln!("[heartmula] {line}"); });
    }
    if let Some(err) = child.stderr.take() {
        thread::spawn(move || for line in BufReader::new(err).lines().map_while(Result::ok) { eprintln!("[heartmula] {line}"); });
    }
    Ok(child)
}
...
fn start_owned_sidecar(slot: &Arc<Mutex<Option<Child>>>, error_slot: &Arc<Mutex<Option<String>>>) -> Result<(), String> {
    ...
    *slot.lock().unwrap() = Some(child);
    ...
    let error = "The local HeartMuLa service did not become ready within 20 seconds.".to_string();
...
async fn sidecar_http(method: String, path: String, body: Option<String>, content_type: Option<String>) -> Result<SidecarHttpResult, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        match response {
            ...
            Err(error) => Err(format!("The local HeartMuLa service at {url} did not answer ({error})")),
        }
    })
    .await
...
pub fn run() {
    ...
    tauri::Builder::default()
            ...
            if owns_sidecar {
                ...
                thread::spawn(move || {
                    loop {
                        ...
                        // A loaded HeartMuLa worker can spend minutes inside GPU
                        ...
                        if !wait_for_port_free(SIDECAR_DOWN_GRACE) {
                            let error = format!("The HeartMuLa service stopped, but port {SIDECAR_PORT} is still occupied.");
        ...
        .expect("error while building HeartMuLa Studio")
        .run(move |_app, event| {
